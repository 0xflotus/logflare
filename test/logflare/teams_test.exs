defmodule Logflare.TeamsTest do
  ...
  test "list_for_user/1 lists all teams of a given user" do
    user = insert(:user)
    teams = insert_list(2, :team_user, email: user.email)

    expected = Enum.map(teams, & &1.team.id) |> Enum.sort()
    result = Enum.map(Teams.list_for_user(user), & &1.id) |> Enum.sort()

    assert expected == result
  end

  test "get/2 gets a team for a given user and token" do
    user = insert(:user)
    team = insert(:team)
    team_user = insert(:team_user, email: user.email, team: team)
    insert_list(2, :team_user)

    assert team.id == Teams.get(user, team.token).id
  end
end
